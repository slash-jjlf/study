...
# 적 enemy 캐릭터
enermy = pygame.image.load("./enermy.png")
enermy_size = enermy.get_rect().size # 이미지의 크기를 구해옴
enermy_width = enermy_size[0] # 적의 가로 크기
enermy_height = enermy_size[1] # 적의 세로 크기
enermy_x_pos = (screen_width / 2) - (enermy_width / 2) # 화면 가로의 절반 크기에 해당하는 곳에 위치
enermy_y_pos = (screen_height / 2)- (enermy_height / 2) # 화면 세로 크기 가장 아래에 해당하는 곳에 위치
